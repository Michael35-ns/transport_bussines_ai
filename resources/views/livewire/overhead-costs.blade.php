<section class="w-full">
    <div class="flex items-center justify-between gap-4">
        <div>
            <flux:heading size="xl">{{ __('Costos generales') }}</flux:heading>
            <flux:subheading>{{ __('Gastos de la empresa (salarios, cargas sociales, admin) — el pool que se reparte entre camiones.') }}</flux:subheading>
        </div>

        @can('create', \App\Models\OverheadCost::class)
            ...
            <flux:table.rows>
                @forelse ($this->costs as $cost)
                    <flux:table.row :key="$cost->id">
                        <flux:table.cell variant="strong">{{ $cost->costType->name }}</flux:table.cell>
                ...
                @empty
                    <flux:table.row>
                        <flux:table.cell colspan="5" class="text-center text-zinc-500">
                            {{ __('No hay costos generales registrados todavía.') }}
                        </flux:table.cell>
                    </flux:table.row>
                @endforelse
            </flux:table.rows>
    ...
    <flux:modal name="overhead-cost-form" class="md:w-[28rem]" @close="resetForm">
        <form wire:submit="save" class="space-y-6">
            <flux:heading size="lg">
                {{ $editingId ? __('Editar costo general') : __('Nuevo costo general') }}
            </flux:heading>
