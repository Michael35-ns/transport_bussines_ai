<section class="w-full">
    <div class="flex items-center justify-between gap-4">
        <div>
            ...
            <flux:subheading>{{ __('Compras de combustible por camión.') }}</flux:subheading>
        </div>
    ...
    <div class="mt-6">
        <flux:table :paginate="$this->records">
            <flux:table.columns>
                ...
                <flux:table.column>{{ __('Estación') }}</flux:table.column>
                <flux:table.column>{{ __('Fecha') }}</flux:table.column>
                <flux:table.column align="end">{{ __('Litros') }}</flux:table.column>
            ...
            <flux:table.rows>
                @forelse ($this->records as $record)
                    <flux:table.row :key="$record->id">
                        ...
                        <flux:table.cell>{{ $record->fuelStation->name }}</flux:table.cell>
                        <flux:table.cell>{{ $record->occurred_at->format('d/m/Y H:i') }}</flux:table.cell>
                        <flux:table.cell align="end">{{ number_format((float) $record->liters, 2) }}</flux:table.cell>
                        <flux:table.cell align="end">₡{{ number_format((float) $record->total, 2) }}</flux:table.cell>
                ...
                @empty
                    <flux:table.row>
                        <flux:table.cell colspan="6" class="text-center text-zinc-500">
                            {{ __('No hay registros de combustible todavía.') }}
                        </flux:table.cell>
                    </flux:table.row>
                @endforelse
            </flux:table.rows>
    ...
    <flux:modal name="fuel-record-form" class="md:w-[36rem]" @close="resetForm">
        <form wire:submit="save" class="space-y-6">
            <flux:heading size="lg">
                {{ $editingId ? __('Editar registro') : __('Nuevo registro de combustible') }}
            </flux:heading>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <flux:select wire:model="fuel_station_id" :label="__('Estación')" required>
                    <flux:select.option value="">{{ __('Seleccione una estación') }}</flux:select.option>
                    @foreach ($this->fuelStationOptions as $id => $name)
                ...
                <flux:input wire:model.blur="liters" :label="__('Litros')" type="number" step="0.0001" required />
                <flux:input wire:model.blur="unit_price" :label="__('Precio por litro (₡)')" type="number" step="0.0001" required />
